...
void IbkrClient::reqCurrentTime()
{
...
void IbkrClient::tickDataOperation()
{
	//! [reqmarketdatatype]
...
void IbkrClient::tickOptionComputationOperation()
{
	/*** Requesting real time market data ***/
	std::this_thread::sleep_for(std::chrono::seconds(1));
...
void IbkrClient::delayedTickDataOperation()
{
...
void IbkrClient::marketDepthOperations()
{
...
void IbkrClient::marketDataType()
{
	//! [reqmarketdatatype]
...
void IbkrClient::historicalDataRequests()
{
...
void IbkrClient::optionsOperations()
{
	//! [reqsecdefoptparams]
...
void IbkrClient::contractOperations()
{
...
void IbkrClient::marketScanners()
{
	/*** Requesting all available parameters which can be used to build a scanner request ***/
	//! [reqscannerparameters]
	m_pClient->reqScannerParameters();
...
void IbkrClient::accountOperations()
{
...
void IbkrClient::orderOperations()
{
...
void IbkrClient::conditionSamples()
{
...
void IbkrClient::testAlgoSamples() {
	//! [algo_base_order]
...
void IbkrClient::financialAdvisorOrderSamples()
{
	//! [faorderoneaccount]
...
void IbkrClient::financialAdvisorOperations()
{
...
void IbkrClient::testDisplayGroups() {
	//! [querydisplaygroups]
...
void IbkrClient::reqFamilyCodes()
{
...
void IbkrClient::reqMatchingSymbols()
{
...
void IbkrClient::reqMktDepthExchanges()
{
...
void IbkrClient::reqSmartComponents()
{
...
void IbkrClient::reqNewsArticle()
{
...
void IbkrClient::reqHistoricalNews() {
...
void IbkrClient::reqHeadTimestamp() {
	//! [reqHeadTimeStamp]
...
void IbkrClient::reqHistogramData() {
	//! [reqHistogramData]
...
void IbkrClient::rerouteCFDOperations()
{
	//! [reqmktdatacfd]
...
void IbkrClient::marketRuleOperations()
{
...
void IbkrClient::continuousFuturesOperations()
{
...
void IbkrClient::reqHistoricalTicks()
{
	//! [reqhistoricalticks]
...
void IbkrClient::wshCalendarOperations() {
	//! [reqmetadata]
...
//! [error]
void IbkrClient::error(int id, time_t errorTime, int errorCode, const std::string& errorString, const std::string& advancedOrderRejectJson)
{
...
void IbkrClient::tickOptionComputation(TickerId tickerId, TickType tickType, int tickAttrib, double impliedVol, double delta,
...
void IbkrClient::tickEFP(TickerId tickerId, TickType tickType, double basisPoints, const std::string& formattedBasisPoints,
...
void IbkrClient::printOrderAllocationsList(const OrderAllocationListSPtr& orderAllocationList) {
...
void IbkrClient::printContractMsg(const Contract& contract) {
	printf("\tConId: %ld\n", contract.conId);
...
void IbkrClient::printContractDetailsMsg(const ContractDetails& contractDetails) {
...
void IbkrClient::printContractDetailsSecIdList(const TagValueListSPtr& secIdList) {
...
void IbkrClient::printTagValueList(const TagValueListSPtr& tagValueList, std::string listName) {
...
void IbkrClient::printConditions(const std::vector<std::shared_ptr<OrderCondition>> conditions) {
...
void IbkrClient::printSoftDollarTier(const SoftDollarTier& softDollarTier) {
...
void IbkrClient::printDeltaNeutralContract(DeltaNeutralContract* deltaNeutralContract) {
...
void IbkrClient::printComboLegs(const Contract::ComboLegListSPtr& comboLegList) {
...
void IbkrClient::printOrderComboLegs(const Order::OrderComboLegListSPtr& orderComboLegList) {
...
void IbkrClient::printContractDetailsIneligibilityReasonList(const IneligibilityReasonListSPtr& ineligibilityReasonList) {
...
void IbkrClient::printBondContractDetailsMsg(const ContractDetails& contractDetails) {
...
//! [updatemktdepth]
void IbkrClient::updateMktDepth(TickerId id, int position, int operation, int side,
...
//! [updatemktdepthl2]
void IbkrClient::updateMktDepthL2(TickerId id, int position, const std::string& marketMaker, int operation,
...
//! [realtimebar]
void IbkrClient::realtimeBar(TickerId reqId, long time, double open, double high, double low, double close,
...
void IbkrClient::commissionAndFeesReport(const CommissionAndFeesReport& commissionAndFeesReport) {
...
//! [accountsummary]
void IbkrClient::accountSummary(int reqId, const std::string& account, const std::string& tag, const std::string& value, const std::string& currency) {
...
void IbkrClient::verifyAndAuthMessageAPI(const std::string& apiDatai, const std::string& xyzChallenge) {
...
//! [positionmulti]
void IbkrClient::positionMulti(int reqId, const std::string& account, const std::string& modelCode, const Contract& contract, Decimal pos, double avgCost) {
...
void IbkrClient::securityDefinitionOptionalParameter(int reqId, const std::string& exchange, int underlyingConId, const std::string& tradingClass,
...
//! [pnlsingle]
void IbkrClient::pnlSingle(int reqId, Decimal pos, double dailyPnL, double unrealizedPnL, double realizedPnL, double value) {
...
//! [tickbytickalllast]
void IbkrClient::tickByTickAllLast(int reqId, int tickType, time_t time, double price, Decimal size, const TickAttribLast& tickAttribLast, const std::string& exchange, const std::string& specialConditions) {
	char timeStr[80];
#if defined(IB_WIN32)
...
//! [tickbytickbidask]
void IbkrClient::tickByTickBidAsk(int reqId, time_t time, double bidPrice, double askPrice, Decimal bidSize, Decimal askSize, const TickAttribBidAsk& tickAttribBidAsk) {
	char timeStr[80];
#if defined(IB_WIN32)
	ctime_s(timeStr, sizeof(timeStr), &time);
#else
...
//! [orderbound]
void IbkrClient::orderBound(long long permId, int clientId, int orderId) {
...
//! [replacefaend]
void IbkrClient::replaceFAEnd(int reqId, const std::string& text) {
...
//! [historicalSchedule]
void IbkrClient::historicalSchedule(int reqId, const std::string& startDateTime, const std::string& endDateTime, const std::string& timeZone, const std::vector<HistoricalSession>& sessions) {
...
//! [userInfo]
void IbkrClient::userInfo(int reqId, const std::string& whiteBrandingId) {
	printf("User Info. ReqId: %d, WhiteBrandingId: %s\n", reqId, whiteBrandingId.c_str());
}
//! [userInfo]

//! [currenttimeinmillis]
void IbkrClient::currentTimeInMillis(time_t timeInMillis) {
	struct tm timeinfo;
...
// protobuf
#if !defined(USE_WIN_DLL)
void IbkrClient::execDetailsProtoBuf(const protobuf::ExecutionDetails& executionDetailsProto) {}
void IbkrClient::execDetailsEndProtoBuf(const protobuf::ExecutionDetailsEnd& executionDetailsEndProto) {}
void IbkrClient::orderStatusProtoBuf(const protobuf::OrderStatus& orderStatusProto) {}
void IbkrClient::openOrderProtoBuf(const protobuf::OpenOrder& openOrderProto) {}
void IbkrClient::openOrdersEndProtoBuf(const protobuf::OpenOrdersEnd& openOrderEndProto) {}
void IbkrClient::errorProtoBuf(const protobuf::ErrorMessage& errorProto) {}
#endif